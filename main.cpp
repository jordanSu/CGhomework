...
GLuint VAO, VBO;
std::string nowShading = "Phong Shading";

std::vector<object_struct> objects;	// VAO: vertex array object,vertex buffer object and texture(color) for objs
unsigned int FlatProgram, GouraudProgram, PhongProgram, BlinnProgram, textProgram;		// Four shader program
...
void RenderText(unsigned int program, std::string text, GLfloat x, GLfloat y, GLfloat scale, glm::vec3 color)
{
    ...
    glUseProgram(program);
	setUniformMat4(program, "projection", glm::ortho(0.0f, 800.0f, 0.0f, 600.0f));
...
static void render()
{
	...
	RenderText(textProgram, nowShading, 570.0f, 570.0f, 0.5f, glm::vec3(1.0, 1.0f, 1.0f));
	// draw every object
	for(int i=0;i<objects.size();i++){
...
static void changeProgram()
{
	switch(ProgramIndex) {
		...
		case 3:	// change to Blinn
			...
			ProgramIndex = 4;
			nowShading = "Blinn-Phong Shading";
			break;
...
int main(int argc, char *argv[])
{
	...
	textProgram = setup_shader(readfile("vsText.txt").c_str(), readfile("fsText.txt").c_str());
	FT_Done_Face(face);
	FT_Done_FreeType(ft);
